...
*** INSTRUCTOR : Gamradt                                          ***
*********************************************************************
*** DESCRIPTION : This ProcessIndexedAddressing.cs file processes ***
***               the rules included for indexed addressing.      ***
********************************************************************/
...
{
    /********************************************************************
    *** CLASS    : Process Indexed Addressing Class                   ***
    *** DESCRIPTION : This class consists of all the functions that   ***
    ***               are used to process indexed addressing.         ***
    *********************************************************************/
    class ProcessIndexedAddressing
    {
        Node foundSymbol;
        Node symbolToAdd;
        ...
        *********************************************************************/
        public void Process(ExpressionsLinkedList expressionsLinkedList, BinarySearchTree symbolTable, string expression)
        {
            ...
            if (tempStrippedStatement[1] == "x" || tempStrippedStatement[1] == "X")
            {
                        ...
                        if (!dupestring)
                        {
                            ...
                            else if (!tempStrippedStatement[0].Any(x => char.IsLetter(x)) && tempStrippedStatement[1].Any(x => char.IsLetter(x)))
                            {
                                ...
                                if (foundSymbol != null)
                                {
                                    if (foundSymbol.Rflag == false)
                                    {
                                        ...
                                        ExpressionNode node = new ExpressionNode(expression, final, false, true, true, true, true);
                            ...
                            else if (tempStrippedStatement[0].Any(x => char.IsLetter(x)) && tempStrippedStatement[1].Any(x => char.IsLetter(x)))
                            {
                                ...
                                if (symbolToAdd != null && foundSymbol != null)
                                {
                                    ...
                                    if (symbolToAdd.Rflag)
                                    {
                                        ...
                                        {
                                            ExpressionNode node = new ExpressionNode(expression, final, true, true, true, true, true);
                                            expressionsLinkedList.Add(node);
                                        }
                                    }
                                    else
                            ...
                            else if (tempStrippedStatement[0].Any(x => char.IsLetter(x)) && !tempStrippedStatement[1].Any(x => char.IsLetter(x)))
                            {
                                ...
                                if (foundSymbol != null)
                                {
                                    ...
                                    ExpressionNode node = new ExpressionNode(expression, final, foundSymbol.Rflag, true, true, true, true);
                                    expressionsLinkedList.Add(node);
                                }
                                else
                                {
                    ...
                    if (tempStrippedStatement[0].Length < 17)
                    {
                        ...
                        if (!dupestring)
                        {
                            if (substring.Any(x => char.IsLetter(x)))
                            {
                                ...
                                try
                                {
                                    ExpressionNode node = new ExpressionNode(expression, symbolToAdd.Value, symbolToAdd.Rflag, true, true, true, true);
                                    expressionsLinkedList.Add(node);
                                }
                                catch
                                {
            ...
            {
                Console.WriteLine("Error: " + expression + " contains an invalid operation. Must be ',x' or ',X'");
            }
            return;
        }
    }
